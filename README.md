...
2. SQL Editor で次の順に実行:
   ...
   17. `supabase/migrations/0115_hm_judgment_rules.sql` …… 健診結果の自動判定基準（A〜D）
   18. `supabase/migrations/0116_hm_judgment_rules_2026.sql` …… 判定区分（2026年4月1日改定）の数値へ差し替え
...
1. 本番（ストレスチェックWebと共用のSupabaseプロジェクト）のSQL Editorで
   `supabase/migrations/0101〜0104, 0106〜0116` **のみ** を順に実行する
