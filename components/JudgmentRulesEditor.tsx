        ...
        <div key={item.key} className="card" style={{ background: "#fbfdfd" }}>
          ...
          {item.textJudged ? (
            <p className="muted">
              ...
              {item.key === "chest_xray" &&
                "所見があればB、腫瘍・腫瘤・結節・空洞・粒状・網状・すりガラス・胸水・狭窄・動脈瘤・蜂巣・気腫・うっ血・リンパ節腫大・縦隔拡大・浸潤・コンソリデーション・胸膜肥厚・胸膜プラーク・破壊・溶骨・骨折・心拡大・癌(がん)・転移・結核・無気肺・肺炎・胸膜炎の語句を含めばDとします（「右胸水」「肺がん疑い」「腹部大動脈瘤」も該当）。「陳旧性」の所見は治癒後の変化としてBにします（陳旧性肺結核 → B）。"}
            </p>
          ) : item.rules.length === 0 ? (
