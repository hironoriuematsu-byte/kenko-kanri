...
// 年度内の産業医巡視記録をまとめて表示・印刷/PDF (company)
export default async function CompanyPatrolsReportPage({
  searchParams,
}: {
  ...
  await supabase.rpc("hm_log_access", {
    ...
    p_target_id: null,
    p_detail: { company_id: profile.company_id, fiscal_year: year, count: patrols.length },
  });
  ...
  return (
    <>
      ...
      <main className="container">
        ...
        >
          <Link className="muted" href={`/company/patrols?year=${year}`}>
            ← 一覧に戻る
          </Link>
          <PrintButton />
